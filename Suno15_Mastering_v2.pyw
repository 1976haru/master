...
V2_EQ = {
    "OLD POP": ["highpass=f=24", "equalizer=f=260:t=q:w=1.0:g=-0.3", "equalizer=f=3000:t=q:w=1.0:g=0.5", "equalizer=f=8500:t=q:w=1.0:g=-0.2"],
    "BALLAD": ["highpass=f=25", "equalizer=f=250:t=q:w=1.0:g=-0.4", "equalizer=f=3000:t=q:w=1.0:g=0.5", "equalizer=f=8500:t=q:w=1.0:g=-0.3"],
    "JAZZ": ["highpass=f=22", "equalizer=f=220:t=q:w=1.0:g=-0.2", "equalizer=f=4500:t=q:w=1.0:g=0.2"],
    "CHANSON": ["highpass=f=24", "equalizer=f=240:t=q:w=1.0:g=-0.3", "equalizer=f=2600:t=q:w=1.0:g=0.6", "equalizer=f=8000:t=q:w=1.0:g=-0.4"],
    "R&B": ["highpass=f=25", "equalizer=f=90:t=q:w=0.8:g=0.5", "equalizer=f=260:t=q:w=1.0:g=-0.5", "equalizer=f=3200:t=q:w=1.0:g=0.5"],
    "SOUL": ["highpass=f=24", "equalizer=f=110:t=q:w=0.9:g=0.4", "equalizer=f=350:t=q:w=1.0:g=0.2", "equalizer=f=3000:t=q:w=1.0:g=0.5", "equalizer=f=8500:t=q:w=1.0:g=-0.2"],
    "CHILI EN": ["highpass=f=25", "equalizer=f=85:t=q:w=0.8:g=0.6", "equalizer=f=250:t=q:w=1.0:g=-0.6", "equalizer=f=3000:t=q:w=1.0:g=0.6", "equalizer=f=9000:t=q:w=1.0:g=0.2"],
    "CHILI JP": ["highpass=f=25", "equalizer=f=90:t=q:w=0.8:g=0.5", "equalizer=f=250:t=q:w=1.0:g=-0.5", "equalizer=f=2800:t=q:w=1.0:g=0.5", "equalizer=f=6200:t=q:w=1.0:g=-0.3"],
    "SHOWA JP": ["highpass=f=22", "equalizer=f=260:t=q:w=1.0:g=-0.2", "equalizer=f=2600:t=q:w=1.0:g=0.3", "equalizer=f=7600:t=q:w=1.0:g=-0.3"],
}
...
def apply_v2_genres():
    if "OLD POP" not in legacy.GENRES:
        legacy.GENRES["OLD POP"] = {"label": "OLD POP LOUNGE", "target_i": -14.0, "target_tp": -1.5, "target_lra": 11, "eq": V2_EQ["OLD POP"], "comp": V2_COMP["OLD POP"], "character": V2_CHARACTER["OLD POP"]}
    if "SHOWA JP" not in legacy.GENRES:
        legacy.GENRES["SHOWA JP"] = {"label": "昭和セブンティーズ", "target_i": -14.3, "target_tp": -1.5, "target_lra": 11, "eq": V2_EQ["SHOWA JP"], "comp": V2_COMP["SHOWA JP"], "character": V2_CHARACTER["SHOWA JP"]}
    ...
    legacy.GENRES["CHANSON"]["character"] = "프랑스어의 둥근 모음과 대화형 보컬, 어쿠스틱 악기 질감, 절제된 고역과 자연스러운 공간"
    legacy.GENRES["CHILI EN"]["character"] = "가까운 보컬, 타이트한 저역, 도시적 Chill Rap/Urban Soul, 선명한 훅과 과하지 않은 고역"
    legacy.GENRES["CHILI JP"]["character"] = "자연스러운 일본어 모라 리듬, 가까운 저중역 보컬, 타이트한 저역, 아이돌식 과광택 없는 고역"


def build_pre_chain_v2(genre_key, factor=1.0):
    g = legacy.GENRES[genre_key]
    threshold, ratio, attack, release, makeup = g["comp"]
    # Normal first-pass factors remain >=0.65. v3.1 automatic retries may go lower
    # so a track that lost too much LRA can be re-rendered with near-transparent compression.
    factor = min(max(float(factor), 0.30), 1.08)
    ratio = 1.0 + (ratio - 1.0) * factor
    filters = list(g["eq"])
    filters.append(f"acompressor=threshold={threshold}:ratio={ratio:.3f}:attack={attack}:release={release}:makeup={makeup}")
    return ",".join(filters)
...
def limiter_filter(genre_key):
    ...
    return f"alimiter=limit={ceiling_linear:.6f}:attack=5:release=50:level=false:latency=true"


def first_pass_v2(ffmpeg, path, genre_key, factor):
    g = legacy.GENRES[genre_key]
    af = build_pre_chain_v2(genre_key, factor) + f",loudnorm=I={g['target_i']}:TP={g['target_tp']}:LRA={g['target_lra']}:print_format=json"
    rc, _, err = legacy.run_ffmpeg(ffmpeg, ["-hide_banner", "-nostats", "-i", str(path), "-af", af, "-f", "null", "-"])
...
def master_two_pass_v2(ffmpeg, src, dst, genre_key, factor, stats):
    ...
    rc, _, err = legacy.run_ffmpeg(ffmpeg, ["-y", "-hide_banner", "-i", str(src), "-af", af, "-ar", "48000", "-c:a", "pcm_s24le", str(dst)])
    return rc == 0, err
